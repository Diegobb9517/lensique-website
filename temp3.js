...
function priceSection(){var q=zeissQuote();if(!q||!q.disponible){return '<section class="sec" style="padding-top:24px"><div class="secK">05 — Tu inversión</div><div style="margin-top:8px;padding:14px 16px;border:1px solid #e7d6b0;border-radius:10px;background:#faf4e6;color:#8a6d2f;font-size:13.5px;line-height:1.5"><b>Esta combinación no está disponible</b><br>'+((q&&q.motivo)||'Ajusta el índice o el tratamiento.')+'</div></section>';}var abbe=q.avisoAbbe?'<div class="muted" style="margin-top:10px;font-size:12px;color:#8a6d2f;text-align:left">ℹ️ '+q.avisoAbbe+'</div>':'';return '<section class="sec" style="text-align:left; padding-top:28px"><div class="secK">05 — Tu inversión</div><div style="font-size:11px;color:var(--grey);letter-spacing:1px;text-transform:uppercase;margin:4px 0 6px">El par · IVA incluido</div><div style="font-family:Georgia,serif;font-size:42px;font-weight:600;color:var(--navy);line-height:1;letter-spacing:.5px">'+money(q.pvp)+'</div>'+(q.pvp>=MSI_MIN?'<div style="display:inline-block;margin-top:10px;padding:8px 16px;border-radius:22px;background:var(--gold);color:#fff;font-size:14.5px;font-weight:700;letter-spacing:.2px">'+MSI+' meses sin intereses · '+money(Math.round(q.pvp/MSI))+'/mes</div>':'')+'<div class="muted" style="margin-top:8px">'+q.etiqueta+' · índice '+q.indice+'</div><div class="muted" style="margin-top:2px">Incluye armado y protección UV · entrega '+q.entrega+' ('+q.tipoFab+')</div>'+abbe+'</section>';}
  ...
  function rxBlock(){
    ...
    if (ST.rxMethod === 'upload') {
      ...
      h += '<div style="margin-bottom: 16px; text-align: left;">';
      h += '<button type="button" onclick="toggleRxGuide(event)" style="background:#f1f5f9;color:var(--navy);border:1px solid #cbd5e1;padding:8px 14px;border-radius:20px;font-size:13px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4"/><path d="M12 8h.01"/></svg><span id="rxGuideLabel">'+(ST.rxGuide?"Ocultar guía para leer receta":"Guía: ¿Cómo leer mi receta?")+'</span></button>';
      h += '</div>';
      h += '<div id="rxGuideWrapper" style="display:'+(ST.rxGuide?'block':'none')+'">';
      h += rxGuide();
      h += '</div>';
...
function toggleRxGuide(e) {
  if (e) e.preventDefault();
  ST.rxGuide = !ST.rxGuide;
  render();
}
